\input{../../templates/header.tex}
\title{Lista 5 - Física 2}
\author{Alessandro Wagner Palmeira}
\begin{document}
\maketitle

\section{Ex. 1}
O buraco também expande, como se fosse feito pelo material. É fácil perceber
isso imaginando um cubo de moléculas ligadas por molas. Quando aquecemos o material,
o comprimento das molas aumenta e o buraco interno também aumenta.

\section{Ex. 2}

Seja $S$ a área de um quadrado de lados $x$ e $y$.

Aumentando a temperatura em $\Delta T$, o lado x aumenta em $\Delta x$ e o lado y em $\Delta y$.

$\Delta S = y \Delta x + x \Delta y + \Delta x \Delta y$ \\
$\Delta S = y \alpha x\Delta T + x \alpha y \Delta T + (\alpha x\Delta T)(\alpha y \Delta T)$ \\
$\Delta S = 2xy\alpha \Delta T + xy\alpha^2\Delta T^2 $ \\
$\Delta S = 2S_{0}\alpha \Delta T + S_{0}\alpha^2\Delta T^2 $ \\

Como $\alpha << 1, \alpha^2\approx0$

Então:

\boxed{\Delta S = 2\alpha S_{0}\Delta T}\\	

\section{Ex. 3}

Dados: \\
$ \left\{
	  \begin{array}{l l}
		  T_{1} = -78,5ºC \\
		  P_{1}=0,900atm \\
	  \end{array}
  \right.
$ \\
$ \left\{
  	\begin{array}{l l}
	  	T_{2} = 78,0ºC \\
      P_{1}=1,635atm 
	  \end{array}
  \right.
$ \\

Ligando os pontos com uma reta temos que o $x$ com $y=0$ é: \\ \\
$\frac{1,635}{78+78,5+x} = \frac{0,9}{x}$ \\ \\
$(1,635-0,9)x = 0,9 * 156,5$\\ \\
$x = 191,63$\\ \\

Portanto o \textit{zero absoluto} será em \boxed{-78,5 - x = -270,13ºC}\\

\section{Ex. 4}

Dados: \\
$ \left\{
	  \begin{array}{l l}
		  T_{1} = 20ºC \\
		  l_{1}=10cm \\
	  \end{array}
  \right.
$ \\
$ \left\{
  	\begin{array}{l l}
		  T_{2} = 100ºC \\
		  l_{2}=10,015cm \\
	  \end{array}
  \right.
$ \\

Para $\Delta T=80ºC$ temos $\Delta L=0,015cm$\\
$\Delta L = \alpha l_{0}\Delta T$\\
Logo, $\alpha = \frac{0,015cm}{10cm*80ºC} = 1,875\e{-5}ºC^{-1}$\\
$ l = l_{0}(1 + \alpha\Delta T)$\\
Item a)\\
$l = 10(1+1,875\e{-5}*-20)$\\
\boxed{l = 9,99625cm}\\
Item b)\\
$\Delta L = \alpha l_{0}\Delta T$\\
$T_{f} - T_{i} = \Delta T = \frac{\Delta L}{\alpha l_{0}} = \frac{0,009}{1,875\e{-5}*10} = 48ºC$\\
$T_{f} = 48ºC + 20ºC$\\
\boxed{T_{f} = 68ºC}\\

\section{Ex. 5}
Supondo que a pessoa seja feita de água, temos:
%TODO
\boxed{\Delta Q = 160Kcal}

\section{Ex. 6}
Calor para esfriar a água: 26150J\\
Calor para esquentar o gelo: m * 42000J\\
Calor para derreter o gelo: m * 334000J\\
Igualando temos:\\
%TODO
\boxed{m = 69,5g}

\section{Ex. 7}
%TODO
\boxed{m = 110g}

\section{Ex. 8}
Congelamento da água:\\
W é positivo pois a água aumenta de volume.\\
Q é negativo pois o calor é tirado para que ele congele.\\
Como \boxed{\Delta U = Q - W}, então: $\Delta U$ é negativo.

\section{Ex. 9}
%TODO
\boxed{W = 23,3J}

\section{Ex. 10}
%TODO
a) \boxed{W = 6cal}\\
b) \boxed{Q = -43cal}\\
c) \boxed{U_{f} = 40cal}\\
d) \boxed{Q_{ib}=Q_{bf} = 18cal}

\end{document}
